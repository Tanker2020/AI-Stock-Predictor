...
def getPandData(num):
    ...
    for i in range(len(stocks)):
        ...
        for i in range(0, len(stocks[currStock]['avgprice'])):
            if (stocks[currStock]['avgprice'][i] != 0):
                temperar = stocks[currStock]["avgprice"][i]
                break
        ...
        dataFilter(currStock,num)
        AI(currStock,data,temperar)
        ...
        try:
            for c in range(len(data[i]['feed'])):
                ...
                if (timeData.timestamp() not in stocks[currStock]['Time']):
                    ...
                    stocks[currStock]['Sentiment'].insert(index,int(float(data[i]['feed'][c]['overall_sentiment_score'])))
                else:
                    index = stocks[currStock]['Time'].index(timeData.timestamp())
...
def AI(currStock,dataframe,x):
    ...
    plot_loss(history,currStock)
    
    #model prediction    
    yPredict = model.predict(test_features).flatten()
    
    for i in range(len(yPredict)):
        yPredict[i] =yPredict[i]+x   
    
    y = yPredict.tolist()
    
    plt.axes(aspect='equal')
    plt.scatter(test_labels, yPredict)
    plt.xlabel(f'True Values of Price for {currStock}')
    plt.ylabel(f'Predictions of Price for {currStock}')
    lims = [75, 200]
    plt.xlim(lims)
    plt.ylim(lims)
    plt.plot(lims, lims)
    ...
    for i in range(len(y)):
        if(i>30):
            break
        with open(f"ouput_{currStock}.txt", "a") as f:
            f.write(f"Day {i}, Predicted Price: {y[-i]} \n")
...
def dataFilter(currStock,num):
    if num == 0:
        #level IV starter data(only 1 data point of avgprice)
        for i in range(len(stocks[currStock]['avgprice'])):
            stocks[currStock]['Sentiment'][i] = 0
            stocks[currStock]['volume'][i] = 0
            stocks[currStock]['Income'][i] = 0
            stocks[currStock]['GDP'][i] = 0
            stocks[currStock]['Cash_Flow'][i] = 0
            stocks[currStock]['Inflation'][i] = 0
            stocks[currStock]['Unemployment'][i] = 0
    elif num == 1:
        #doesn't use GDP data and Sentiment data and Income data and Cash Flow data and Inflation data and Unemployment data
        for i in range(len(stocks[currStock]['avgprice'])):
            stocks[currStock]['Sentiment'][i] = 0
            stocks[currStock]['Income'][i] = 0
            stocks[currStock]['GDP'][i] = 0
        ...
        #doesn't use GDP data and Sentiment data and Unemployment data and Income data and Cash Flow data
        for i in range(len(stocks[currStock]['avgprice'])):
            stocks[currStock]['Sentiment'][i] = 0
            stocks[currStock]['Income'][i] = 0
            stocks[currStock]['GDP'][i] = 0
        ...
        #doesn't use GDP data and Sentiment data and Cash Flow data and Income data
        for i in range(len(stocks[currStock]['avgprice'])):
            stocks[currStock]['Sentiment'][i] = 0
            stocks[currStock]['Income'][i] = 0
            stocks[currStock]['GDP'][i] = 0
        ...
        #doesn't use GDP data and Sentiment data and Income data
        for i in range(len(stocks[currStock]['avgprice'])):
            stocks[currStock]['Sentiment'][i] = 0
            stocks[currStock]['Income'][i] = 0
            stocks[currStock]['GDP'][i] = 0
    elif num == 5:
        #doesn't use GDP data and Sentiment data
        for i in range(len(stocks[currStock]['avgprice'])):
            stocks[currStock]['Sentiment'][i] = 0
            stocks[currStock]['GDP'][i] = 0
    elif num == 6:
        #doesn't use Sentiment data
        for i in range(len(stocks[currStock]['avgprice'])):
            stocks[currStock]['Sentiment'][i] = 0
    elif num == 7:
        #gold standard data level of IV (all data points)
        pass

def plot_loss(history,currstock):
  plt.plot(history.history['loss'], label='loss')
  plt.plot(history.history['val_loss'], label='val_loss')
  plt.ylim([0, 500])
  plt.xlabel('Epoch')
  plt.ylabel(f'Error [{currstock} Prices]')
  plt.legend()
  plt.grid(True)
  plt.show()

if __name__ == "__main__":
    num = int(input("How many datasets do you want to use: "))
    getPandData(num)